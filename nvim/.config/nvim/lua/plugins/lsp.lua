...
vim.api.nvim_create_autocmd("LspAttach", {
  ...
  callback = function(event)
    ...
    -- on_jump is handled globally in diagnostics.lua
    map("[d", function()
    ...
    -- Native snippet navigation (vim.snippet, nvim 0.12)
    vim.keymap.set({ "i", "s" }, "<Tab>", function()
      if vim.snippet.active({ direction = 1 }) then
        return "<Cmd>lua vim.snippet.jump(1)<CR>"
      end
      return "<Tab>"
    end, { buffer = buf, expr = true, desc = "Snippet jump forward" })
    vim.keymap.set({ "i", "s" }, "<S-Tab>", function()
      if vim.snippet.active({ direction = -1 }) then
        return "<Cmd>lua vim.snippet.jump(-1)<CR>"
      end
      return "<S-Tab>"
    end, { buffer = buf, expr = true, desc = "Snippet jump backward" })
  end,
})
