...
},
    {
      ...
      config = function()
        require('nvim-treesitter.configs').setup {
          ensure_installed = {
            'terraform', 'python', 'bash', 'dockerfile', 'yaml', 'json', 'markdown'
          },
    ...
    },
    {
      ...
      config = function()
        require('mason-lspconfig').setup {
          ensure_installed = {
            'terraformls', 'pyright', 'bashls', 'dockerls', 'yamlls', 'jsonls'
          }
        }
      end,
    },
{
  ...
  config = function()
    -- YAML: kubernetes and docker-compose schemas for *.k8s.yaml and docker-compose*.yml
    ...
    -- Enable all key LSPs (yamlls, pyright, bashls, dockerls, jsonls, terraformls)
...
-- improved, mnemonic, conflict-free keymaps for trouble.nvim
